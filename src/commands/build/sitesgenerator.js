...
exports.SitesGenerator = class {
  ...
  async generate(jsonEnvVars=[]) {
      ...
      if (isValidFile(filename)) {
        ...
        try {
          ...
          if (e instanceof SyntaxError) {
            throw new UserError(`JSON SyntaxError: could not parse ${path}`, err.stack);
          } else {
            throw new SystemError(e.message, e.stack);
          }
        }
      }
    });
    ...
    try {
      ...
      throw new SystemError("Failed to read partials", err.stack);
    }
